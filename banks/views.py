...
def complete_profile(request):
    if request.method == 'POST':
        ...
        if user_id and user_username and name and email and mobile and address:
            ...
            context = {
                'msg':"Enter Valid Credentials"}
            return render(request,"user_complete_profile.html",context)
...
def user_settings_member_blood_bank(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            print(all([blood_groups , blood_bank_name , address , state , city , mobile , email , category , status , website , A_positive_units , A_negative_units , B_positive_units , B_negative_units , AB_positive_units , AB_negative_units , O_positive_units , O_negative_units , request_message]))
            if all([blood_groups , blood_bank_name , address , state , city , mobile , email , category , status , website , A_positive_units , A_negative_units , B_positive_units , B_negative_units , AB_positive_units , AB_negative_units , O_positive_units , O_negative_units , request_message]):
                r = AddStaffRequest(
                    user_id = request.user.id,
                    ...
                    A_positive_units = A_positive_units,
                    A_negative_units = A_negative_units,
                    B_positive_units = B_positive_units,
                    B_negative_units = B_negative_units,
                    AB_positive_units = AB_positive_units,
                    AB_negative_units = AB_negative_units,
                    O_positive_units = O_positive_units,
                    O_negative_units = O_negative_units,
                    date_send = datetime.datetime.now().date(),
                    request_message = request_message
                )
                r.save()
                ...
                context = {
                    'msg':"Enter Valid Credentials"}
                return render(request,"user_add_blood_bank.html",context)
...
# DESC : this page allows the user to open and view complete message
...
def cancel_appoinement(request,id):
    if request.user.is_authenticated:
        data = Appointments.objects.get(id=id)
        data.delete()
...
def user_admin_membership_requests(request):
    ...
    # return HttpResponse("Invaid User")
    ...
    if request.user.is_staff:
        ...
        msg = MessageUser(
            ...
            message = '''[Appointment Request Declined]
                        Bank ,
                        Sorry !! .
                        Your appointment request is declined.'''
        )
        msg.save()  
        data.delete()
        return redirect('/user/home/')
